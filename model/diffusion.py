...
class GaussianDiffusion(nn.Module):
    ...
    def predict_start_from_noise(self, x_t, t, noise):
        ...
        if self.predict_epsilon:
            return (
                    extract(self.sqrt_recip_alphas_cumprod, t, self.diffusion_dims) * x_t[:, self.diffusion_dims] -
                    extract(self.sqrt_recipm1_alphas_cumprod, t, self.diffusion_dims) * noise[:, self.diffusion_dims]
            )
        else:
            return noise
    ...
    def q_posterior(self, x_start, x_t, t):
        ...
        posterior_mean[:, self.diffusion_dims] = (
                extract(self.posterior_mean_coef1, t, x_t.shape) * x_start[:, self.diffusion_dims] +
                extract(self.posterior_mean_coef2, t, x_t.shape) * x_t[:, self.diffusion_dims]
        )
